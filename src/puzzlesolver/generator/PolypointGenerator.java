...
public class PolypointGenerator extends RotationGenerator {
     ...
     * @throws IllegalArgumentException if either parameter is out of bounds, or {@code
     *                                  minXDiffFactor > maxXDiffFactor}
     */
    public PolypointGenerator(double minXDiffFactor, double maxXDiffFactor) {
        if (minXDiffFactor <= 0 || minXDiffFactor > 1 || maxXDiffFactor <= 0
            || maxXDiffFactor > 1) {
            throw new IllegalArgumentException(String.format(
                "Diff factors must be (0, 1], were min: %f, max: %f", minXDiffFactor,
                maxXDiffFactor));
        }
